...
const TOOL_META = {
  ...
  "GET /api/v1/logs/calls": { name: "logs_calls", description: { en: "Recent call log entries.", it: "Voci recenti del log chiamate." }, inputSchema: S({ limit: z.coerce.number().int().optional() }) },
  "GET /api/v1/logs/errors": { name: "logs_errors", description: { en: "Recent error log entries.", it: "Voci recenti del log errori." }, inputSchema: S({ limit: z.coerce.number().int().optional() }) },
  "POST /api/v1/profiles/purge": { name: "profile_purge", description: { en: "Purge a profile column (must be empty).", it: "Elimina la colonna di un profilo (deve essere vuota)." }, inputSchema: S({ profile: z.string() }) },
  "POST /api/v1/pressure/inspect": { name: "pressure_inspect", description: { en: "Inspect cooldown pressure (cooldowns, model bench, leases).", it: "Ispeziona la pressione dei cooldown (cooldown, bench modello, lease)." }, inputSchema: S({ limit: z.coerce.number().int().optional() }) },
  "POST /api/v1/pressure/clear": { name: "pressure_clear", description: { en: "Clear cooldowns for a unique, a model, or all.", it: "Azzera i cooldown per un unique, un modello o tutti." }, inputSchema: S({ unique: z.string().optional(), model: z.string().optional() }) },
  "POST /api/v1/playground": { name: "playground", description: { en: "Simulate a chat and return the routing trace.", it: "Simula una chat e ritorna il trace di routing." }, inputSchema: S({ model: z.string(), messages: z.array(z.any()), profile: z.string().optional(), max_tokens: z.coerce.number().int().optional() }) },
  ...
  // --- osservabilità & statistiche ---
  "GET /api/v1/stats/summary": { name: "stats_summary", description: { en: "Aggregate stats: tokens 1h/24h, cache, success rate, preferred model.", it: "Statistiche aggregate: token 1h/24h, cache, success rate, modello preferito." }, inputSchema: S({}) },
  "GET /api/v1/stats/tokens": { name: "stats_tokens", description: { en: "Token usage by model/profile/day.", it: "Uso token per modello/profilo/giorno." }, inputSchema: S({ window: z.string().optional() }) },
  "GET /api/v1/stats/cache": { name: "stats_cache", description: { en: "Cache hit / coalescing statistics.", it: "Statistiche cache hit / coalescing." }, inputSchema: S({}) },
  "GET /api/v1/stats/models": { name: "stats_models", description: { en: "Model ranking (calls, success, latency, tokens).", it: "Classifica modelli (chiamate, successo, latenza, token)." }, inputSchema: S({ window: z.string().optional() }) },
  "GET /api/v1/stats/deployments": { name: "stats_deployments", description: { en: "Per-deployment statistics.", it: "Statistiche per deployment." }, inputSchema: S({ profile: z.string().optional(), sort: z.string().optional(), order: z.string().optional() }) },
  "GET /api/v1/stats/providers": { name: "stats_providers", description: { en: "Per-provider statistics.", it: "Statistiche per provider." }, inputSchema: S({}) },
  "GET /api/v1/deployments/stats": { name: "deployments_stats", description: { en: "Persisted per-deployment scores (ok/fail, latency EMA).", it: "Punteggi persistiti per deployment (ok/fail, latenza EMA)." }, inputSchema: S({ profile: z.string().optional() }) },
  "GET /api/v1/providers/health": { name: "providers_health", description: { en: "Aggregate provider health: counters, breakers, latencies.", it: "Salute aggregata per provider: contatori, breaker, latenze." }, inputSchema: S({}) },
  "GET /api/v1/stats/sessions": { name: "stats_sessions", description: { en: "Session leaderboard (calls, success, tokens, preferred model).", it: "Classifica sessioni (chiamate, successo, token, modello preferito)." }, inputSchema: S({ window: z.string().optional(), limit: z.coerce.number().optional() }) },
  "GET /api/v1/sessions": { name: "sessions_list", description: { en: "Active sessions: sticky, cache holders, dep guards, slow demoted.", it: "Sessioni attive: sticky, cache holder, dep guard, slow demoted." }, inputSchema: S({}) },
  "GET /api/v1/sessions/:id": { name: "sessions_detail", description: { en: "One session: successful deployment ranking, preferred model, tokens.", it: "Una sessione: classifica deployment con successo, modello preferito, token." }, inputSchema: S({ id: z.string(), window: z.string().optional() }) },
  "GET /api/v1/tuning": { name: "tuning_get", description: { en: "Effective runtime tuning parameters (router/forwarder/admin/storage).", it: "Parametri di tuning effettivi a runtime (router/forwarder/admin/storage)." }, inputSchema: S({}) },
  // --- configurazione (raw) ---
  "GET /api/v1/policy/raw": { name: "policy_raw_get", description: { en: "Raw gateway.yaml policy text.", it: "Testo grezzo della policy gateway.yaml." }, inputSchema: S({}) },
  "PUT /api/v1/policy/raw": { name: "policy_raw_put", description: { en: "Replace the whole gateway.yaml (validated).", it: "Sostituisci l'intera gateway.yaml (validata)." }, inputSchema: S({ raw: z.string() }) },
  "GET /api/v1/csv": { name: "csv_get", description: { en: "Raw deployments CSV + parsed rows.", it: "CSV deployment grezzo + righe parse." }, inputSchema: S({}) },
  "PUT /api/v1/csv": { name: "csv_put", description: { en: "Replace the whole deployments CSV (validated).", it: "Sostituisci l'intero CSV deployment (validato)." }, inputSchema: S({ raw: z.string() }) },
  "GET /api/v1/backups": { name: "backup_list", description: { en: "List config backups.", it: "Elenca i backup di configurazione." }, inputSchema: S({}) },
  "POST /api/v1/backups/restore": { name: "backup_restore", description: { en: "Restore a config backup by filename.", it: "Ripristina un backup di configurazione per filename." }, inputSchema: S({ filename: z.string() }) },
  // --- MCP config protocol ---
  "GET /api/v1/mcp/config/tools": { name: "mcp_config_tools", description: { en: "Catalogue of gateway configuration tools.", it: "Catalogo dei tool di configurazione del gateway." }, inputSchema: S({}) },
  "POST /api/v1/mcp/config/execute": { name: "mcp_config_execute", description: { en: "Execute one gateway configuration tool ({tool, arguments}).", it: "Esegue un tool di configurazione del gateway ({tool, arguments})." }, inputSchema: S({ tool: z.string(), arguments: z.record(z.any()).optional() }) },
  "POST /api/v1/mcp/config/call": { name: "mcp_config_call", description: { en: "JSON-RPC 2.0 call to the gateway config MCP (initialize/tools/list/tools/call).", it: "Chiamata JSON-RPC 2.0 all'MCP config del gateway (initialize/tools/list/tools/call)." }, inputSchema: S({ method: z.string(), params: z.record(z.any()).optional(), id: z.any().optional() }) },
};
