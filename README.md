...
| `stream_slow_race_after_ms` / `nonstream_slow_race_after_ms` / `slow_canary_after_ms` / `slow_race_max_warm` | 45000 / 45000 / 15000 / 6 | slow race: after `*_slow_race_after_ms` without delivery the dep is flagged slow-per-session (warm demote + `go_refund` grant); the substitute canary opens on its OWN `slow_canary_after_ms` (15s, one knob for stream & non-stream; 0 = coupled to the flag); `slow_race_max_warm` gates the canary to sessions with few healthy warms |
...
| `slow_latency_abs_floor_ms` / `slow_latency_rel_mult` / `slow_latency_min_peers` / `slow_rel_baseline_mult` | 30000 / 2.0 / 5 / 2.0 | size-aware "slow" detection: beyond `max(floor, mult × fleet median)`, needs `min_peers` peers; session demote needs `> rel_baseline_mult × baseline` |
...
| `slow_latency_abs_floor_ms` / `slow_latency_rel_mult` / `slow_latency_min_peers` | 30000 / 2.0 / 5 | size-aware "slow" threshold: `max(floor, rel x atteso)` where the expectation is the fleet median in the context bucket -> global median -> prefill/gen rate -> 90s legacy |
