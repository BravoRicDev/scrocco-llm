...
def test_preferisce_stesso_modello_su_altro_provider(r):
    ...
    assert avoid == (0, 0)          # altro provider + stesso modello
    assert avoid_other == (1, 1)    # stesso provider
    dep = r.pick_deployment(f"{BASE}-200k", ctx=100)
    assert dep["unique"] == _dep(r, "K-O1")["unique"]


def test_walk_chain_alterna_provider(r):
    g = _dep(r, "K-G1")
    o = _dep(r, "K-O1")
    r._last_attempt = ("groq", "m/shared")
    got = r._walk_chain([g["unique"], o["unique"]], None, ctx=100)
...
def test_knob_off_disattiva_alternanza(r):
    ...
    r._last_attempt = ("groq", "m/shared")
    assert r._prov_avoid_key(_dep(r, "K-G1")) == (0, 0)
...
def test_ultimo_tentativo_registrato_su_note_start(r):
    ...
    assert r._last_attempt == ("groq", "m/shared")
